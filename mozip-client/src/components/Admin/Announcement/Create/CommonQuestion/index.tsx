...
function CommonQuestion(props: CommonQuestionProps) {
  ...
  const handleAddQuestion = () => {
    ...
    if (type === 'long') {
      // default type is long -> '주관식'
      if (!maxLength)
        return alert(`질문${lastIndex + 1} 최대 글자수를 입력해 주세요.`);
    }
    if (!questionScore)
      return alert(`질문${lastIndex + 1} 배점을 입력해 주세요.`);
  ...
  useEffect(() => {
    setTotal(
      commonQuestions
        .map((v: NoticeQuestion) => v.questionScore)
  ...
  return (
    <Ul>
      ...
      <Li>
        <QuestionAddButton onClick={handleAddQuestion}>
          질문 추가
        </QuestionAddButton>
      </Li>
      <AlignCenter>
        <Button padding="17px 24px;">임시 저장</Button>
        <Button padding="17px 91px;" onClick={handleNextPage}>
          다음
        </Button>
      </AlignCenter>
    </Ul>
